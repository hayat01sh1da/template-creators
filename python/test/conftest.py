...
_TEMPLATE_FILES_GLOB = os.path.join('..', '..', 'working-report', _USERNAME, _YEAR, '**', '*.md')
...
def _cleanup_template_dir():
    ...
    if len(glob.glob(_TEMPLATE_FILES_GLOB, recursive=True)) == 0 and os.path.exists(_BASE_DIR):
